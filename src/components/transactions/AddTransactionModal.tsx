  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Add Transaction" maxWidth="lg">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* Intent Helper Info */}
        {mode === 'PERSONAL' ? (
          <div className="p-2.5 rounded-xl bg-purple-50/80 dark:bg-purple-950/40 border border-purple-200/70 dark:border-purple-900/40 flex items-center gap-2 text-xs text-purple-700 dark:text-purple-300">
            <Wallet className="w-4 h-4 shrink-0" />
            <span>Personal expense only for you. Does not affect any friend debt.</span>
          </div>
        ) : mode === 'INDIVIDUAL' ? (
          <div className="p-3 bg-indigo-50/70 dark:bg-indigo-950/40 rounded-xl border border-indigo-100 dark:border-indigo-900/40 space-y-2">
            <div className="flex items-center gap-1.5 text-xs font-bold text-slate-800 dark:text-slate-200">
              <User className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400" />
              <span>Choose Friend Transaction Type:</span>
            </div>
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                onClick={() => {
                  setPaidBy('ME');
                  setCategory('Friend Loan');
                  if (!description) setDescription('Personal loan');
                }}
                className={`py-2 px-2.5 rounded-xl border text-xs font-bold transition-all text-left flex flex-col gap-0.5 ${
                  paidBy === 'ME'
                    ? 'bg-white dark:bg-slate-900 border-indigo-600 text-indigo-700 dark:text-indigo-300 shadow-sm'
                    : 'border-slate-200 dark:border-slate-800 text-slate-500 hover:bg-white/50'
                }`}
              >
                <span>Giving Loan to Friend</span>
                <span className="text-[10px] font-normal text-slate-400">You paid • Friend owes you</span>
              </button>
              <button
                type="button"
                onClick={() => {
                  setPaidBy(friendId || 'FRIEND');
                  if (!description) setDescription('Friend paid for me');
                }}
                className={`py-2 px-2.5 rounded-xl border text-xs font-bold transition-all text-left flex flex-col gap-0.5 ${
                  paidBy !== 'ME'
                    ? 'bg-white dark:bg-slate-900 border-indigo-600 text-indigo-700 dark:text-indigo-300 shadow-sm'
              ...
              >
                <span>Friend Paid for Me</span>
                <span className="text-[10px] font-normal text-slate-400">Friend paid • You owe friend</span>
              </button>
            </div>
          </div>
        ) : null}
